...
## 2026-09-10 — Vitest 유닛 테스트 도입

- **테스트 프레임워크로 Vitest 선택**: 이 프로젝트 최초의 테스트 프레임워크다. Next.js 16 + Vite 기반 도구 체인과 잘 맞고 설정이 가볍다는 점에서 Jest 대신 선택했다. `@types/node`가 `^20`으로 고정돼 있어 `@types/node ^22`를 요구하는 최신 vitest 5 대신, 호환되는 `vitest@2.1.9`를 명시적으로 설치했다.
- **범위는 순수 함수로 한정**: 이번에 추가한 테스트는 `src/lib/inventory.ts`의 `isLowStock` 경계값(0, 음수, 1, 임계값, 임계값+1, 큰 수)만 검증한다. 이 저장소가 원격 샌드박스 환경에서는 Supabase(`*.supabase.co`)로 나가는 아웃바운드 연결이 조직 네트워크 정책으로 차단돼 있어(프록시 403), UI/Supabase 연동까지 포함하는 테스트는 로컬 환경에서만 실질적으로 검증 가능하다. 그래서 지금은 네트워크·DOM 의존이 없는 순수 로직만 자동화하고, 화면 동작 확인은 기존처럼 로컬에서 직접 브라우저로 하는 방식을 유지한다.
- **경로 별칭 유지**: `vitest.config.ts`에 `tsconfig.json`과 동일한 `@/*` → `src/*` 별칭을 추가해, 테스트 코드도 앱 코드와 동일한 `@/lib/...` import 스타일을 쓸 수 있게 했다.
