...
- **`products` update RLS 정책 추가**: 재고 수정을 위해 "누구나 update 가능" 정책을 `products`에 추가했다(`supabase/schema.sql`). 상품 등록(insert) 때와 동일하게 관리자 인증이 없는 현재 단계의 임시 조치이며, Supabase Auth 도입 시 관리자 역할로 제한해야 한다.
