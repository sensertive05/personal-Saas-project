...
10. 관리자 주문 관리(`/admin/orders`)는 `getAllOrders`(`src/lib/queries/orders.ts`)로 `guest_id` 필터 없이 전체 주문을 조회한다. 상태 변경은 `updateOrderStatus`가 기존 `orders` 테이블의 "누구나 read/write 가능" RLS 정책을 그대로 이용해 `status` 컬럼을 직접 update한다(재고와 달리 노출되면 위험한 다른 컬럼이 없어 별도 RPC 불필요). 고객용 `/orders` 페이지의 `guest_id` 스코프 `["orders"]` 쿼리와 캐시가 섞이지 않도록 `["admin-orders"]`라는 별도 쿼리 키를 사용한다.
