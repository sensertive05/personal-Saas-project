...
`/admin/products`에서 새 상품을 등록할 수 있습니다. `/admin/inventory`에서는 등록된
상품의 재고 수량을 +/- 버튼이나 직접 입력으로 수정할 수 있습니다. 관리자 인증이
아직 없어 누구나 접근/수정 가능한 상태입니다 (`docs/decision-log.md` 참고).
...
관리자(주문 관리/매출 대시보드) → Supabase Auth 기반 인증 →
